...
const TrackDetailsPage = (props: TrackDetailsPageProps) => {
  const [trackDetails, setTrackDetails] = React.useState<TrackInfoResponseType>();
  const [trackAudioFeatures, setTrackAudioFeatures] = React.useState<AudioFeature>();
  const [isDetailsFetched, setIsDetailsFetched] = React.useState<boolean>(false);

  const { id } = useParams();
  const { isLoading } = useQuery(['getTrackInfo', id], () => getTrackDetails(id!), {
  ...
  const {} = useQuery(['getTrackAudioFeatures', id], () => getTrackAudioFeatures(id!), {
